...
export default function GiftPicker({ gift, onChange }) {
  ...
  const handleFile = (file) => {
    ...
    if (file.size > MAX_MB * 1024 * 1024) {
      setError(`That photo is a little large — please keep it under ${MAX_MB}MB.`);
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
      onChange({ type: 'photo', photoFile: file, photoPreview: reader.result, amount: '' });
    };
    reader.readAsDataURL(file);
  };

  const clearPhoto = () => onChange({ type: null, photoFile: null, photoPreview: null });

  return (
    <div className="w-full max-w-sm">
      <p className="mb-3 font-display text-2xl text-wine">What are you gifting your sister?</p>
      ...
      {!gift.photoPreview ? (
        <div
          ...
          className={`focus-ring flex cursor-pointer flex-col items-center justify-center rounded-[3px] border-2 border-dashed px-6 py-10 text-center transition-colors ${
            dragOver ? 'border-saffron bg-saffron/5' : 'border-gold/50 bg-ivory-dim'
          }`}
        >
          <p className="font-display text-lg text-wine">A little surprise for her</p>
        ...
        >
          <img src={gift.photoPreview} alt="Gift preview" className="h-48 w-full object-cover" />
          <button
            onClick={clearPhoto}
      ...
      <label className="block">
        <span className="mb-2 block font-display text-lg text-wine">Enter an amount instead</span>
        <div className="flex items-center rounded-[2px] border-b-2 border-gold/50 focus-within:border-wine">
          ...
          <input
            ...
            inputMode="decimal"
            value={gift.amount || ''}
            onChange={(e) => onChange({ type: e.target.value ? 'amount' : null, amount: e.target.value, photoFile: null, photoPreview: null })}
            placeholder="1100"
