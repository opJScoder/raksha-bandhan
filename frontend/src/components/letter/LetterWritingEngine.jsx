      ...
      parts.forEach((part) => {
        ...
        if (part === '\n') {
          ...
          part.split(' ').forEach((w, i, arr) => {
            if (w === '' ) return;
            tokens.push({ type: 'word', text: w + (i < arr.length - 1 ? ' ' : '') });
          });
        }
      });
    } else {
...
export default function LetterWritingEngine({ segments, mediaRenderers, speedMs = 55, onComplete }) {
  ...
  }, [visibleCount, tokens, done, speedMs, onComplete, play]);
  ...
  useLayoutEffect(() => {
    ...
    setPenPos({
      ...
      y: wordRect.top - containerRect.top - 6,
      visible: !done,
    });
  }, [visibleCount, done]);
