...
export default function Rakhi({ size = 160, sent = false }) {
  return (
    <motion.div
      ...
      animate={sent ? { scale: [1, 1.15, 0], opacity: [1, 1, 0] } : { rotate: [-2, 2, -2] }}
      transition={sent ? { duration: 0.8, ease: 'easeIn' } : { duration: 4, repeat: Infinity, ease: 'easeInOut' }}
    >
      <div
        className="absolute inset-0 rounded-full blur-lg"
        style={{ background: 'radial-gradient(circle, rgba(212,168,79,0.45), transparent 70%)' }}
      />
      <svg viewBox="0 0 160 160" width={size} height={size}>
        ...
        <path d="M8 80 Q40 40 78 78" stroke="url(#thread)" strokeWidth="4" fill="none" strokeLinecap="round" />
        <path d="M152 80 Q120 40 82 78" stroke="url(#thread)" strokeWidth="4" fill="none" strokeLinecap="round" />
        ...
        {/* central medallion */}
        <circle cx="80" cy="80" r="26" fill="url(#medallion)" stroke="#6E1737" strokeWidth="2" />
        <circle cx="80" cy="80" r="10" fill="#6E1737" />
        <circle cx="80" cy="80" r="4" fill="#FFF7E8" />

        {/* small beads along the threads */}
        {[
          [30, 62], [22, 74], [130, 62], [138, 74],
        ].map(([cx, cy], i) => (
          <circle key={i} cx={cx} cy={cy} r="4" fill="#E88932" stroke="#6E1737" strokeWidth="1" />
        ))}
      </svg>
    </motion.div>
  );
